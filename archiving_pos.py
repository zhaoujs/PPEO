...
class mesh_crowd(object):
    def __init__(self, curr_archiving_in, curr_archiving_fit, curr_archiving_syn, mesh_div, min_, max_, particals):
        self.curr_archiving_in = curr_archiving_in  # 当前存档中所有粒子的坐标
        self.curr_archiving_fit = curr_archiving_fit  # 当前存档中所有粒子的fitness
        self.curr_archiving_syn = curr_archiving_syn
        self.mesh_div = mesh_div  # 等分因子，默认值为10等分

        self.num_ = self.curr_archiving_in.shape[0]  # 存档大小
...
class get_gbest(mesh_crowd):
    def __init__(self, curr_archiving_in, curr_archiving_fit, curr_archiving_syn,  mesh_div_num, min_, max_, particals):
        super().__init__(curr_archiving_in, curr_archiving_fit, curr_archiving_syn, mesh_div_num, min_, max_, particals)
        self.divide_archiving()
        self.get_crowd()

    def get_probability(self):
        for i in range(self.num_):  # 遍历存档大小
    ...
    def get_gbest(self):
        ...
        for i in range(self.particals):
            ...
            self.gbest_syn.append(self.curr_archiving_syn[gbest_index])
        self.gbest_syn = np.array(self.gbest_syn)
        return self.gbest_in, self.gbest_fit, self.gbest_syn
...
class clear_archiving(mesh_crowd):
    ...
    def clear_(self, thresh):
        ...
        print("Now %d" % len(self.curr_archiving_in) + "个")
        print("Clearing %d" % len(clear_index) + "个")
        ...
        self.curr_archiving_syn = np.delete(self.curr_archiving_syn, clear_index, axis=0)  # 初始化gbest矩阵_适应值
        print("After %d" % len(self.curr_archiving_in) + "个")

        return self.curr_archiving_in, self.curr_archiving_fit, self.curr_archiving_syn
