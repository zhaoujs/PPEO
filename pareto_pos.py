...
class Pareto_:
    def __init__(self, in_data, fitness_data, syn=None):
    ...
    def next(self):
        ...
        if self.syn is not None:
            return self.in_data[self.cursor], self.fitness_data[self.cursor], self.syn[self.cursor]
    ...
    def remove(self):
        ...
        self.syn = np.delete(self.syn, self.cursor, axis=0)
        # 游标回退一步
        self.cursor = self.cursor - 1
        # 非优解个数，加1
    ...
    def pareto(self):
        while self.hasNext():
            # 获取当前位置的粒子信息
            in_curr, fitness_curr, syn_curr = self.next()
            # 判断当前粒子是否pareto最优
            if not judge_(fitness_curr, self.fitness_data, self.cursor):
                self.remove()
        return self.in_data, self.fitness_data, self.syn
